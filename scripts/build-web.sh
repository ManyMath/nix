#!/usr/bin/env bash
# Build the Flutter web app inside a Nix shell.
...
if [ "${1:-}" = "--refresh" ]; then
    ...
    nix develop "$FLAKE_DIR#web" --refresh --command bash -c "$BUILD_CMD"
else
    echo "Building with pinned nixpkgs (from flake.lock)..."
    nix develop "$FLAKE_DIR#web" --command bash -c "$BUILD_CMD"
fi
